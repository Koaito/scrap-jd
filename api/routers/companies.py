...
def get_partnership_signals(
    ...
    for cid in company_id or []:
        if not db_module.is_valid_uuid(cid):
            raise HTTPException(status_code=400, detail={"error_code": error_codes.COMPANY_COMPANY_ID_INVALID_UUID, "message": f"company_id '{cid}' không đúng định dạng UUID.", "params": {"value": cid}})
